...
class CompSystem : public System {
  ...
  public:
    ...
    bool update() override {
      ...
      if(radio.ready()) {
        if(receive_message(&message_buffer, &message_bytes_received)) {
          
          // ---- reply with telemetry --- //
          Serial.print("Received message (Packet: ");
          Serial.print(message_buffer.packet_number);
          Serial.println("), replying with telemetry");
          message_buffer = Message_init_zero;
          if(imu_success) imu_data_to_msg(&message_buffer, &imu_data);
          if(enviro_success) enviro_data_to_msg(&message_buffer, &enviro_data);
          if(gps_success) gps_data_to_msg(&message_buffer, &gps_data);
          send_telemetry(&message_buffer, &radio_data, packet_number++);
  ...
  private:
    ...
    uint8_t message_bytes_received = 0;
